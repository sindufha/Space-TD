...
# Dictionary untuk menyimpan tower instances berdasarkan grid position
...
func setup_selected_tower_ui():
	# Fungsi ini akan dipanggil saat _ready(), tapi UI belum ada
	# Jadi kita skip setup untuk sekarang, nanti dihandle saat UI sudah dibuat
	pass

func show_selected_tower_info(tower_instance: Node, grid_pos: Vector2i):
	# Cek apakah SelectedTowerUI sudah ada
	if not has_node("SelectedTowerUI"):
		print("SelectedTowerUI belum dibuat")
		return
	...
	# Ambil stats dari tower instance
	...
	# Set data ke UI labels
	# Asumsi struktur UI: SelectedTowerUI memiliki child labels dengan nama berikut:
	if selected_tower_ui.has_node("TowerNameLabel"):
		selected_tower_ui.get_node("TowerNameLabel").text = tower_name
	
	if selected_tower_ui.has_node("DamageLabel"):
		selected_tower_ui.get_node("DamageLabel").text = "Damage: " + damage
	
	if selected_tower_ui.has_node("RangeLabel"):
		selected_tower_ui.get_node("RangeLabel").text = "Range: " + range_text
	
	if selected_tower_ui.has_node("AttackSpeedLabel"):
		selected_tower_ui.get_node("AttackSpeedLabel").text = "Attack Speed: " + attack_speed
...
func get_tower_index_from_instance(tower_instance: Node) -> int:
	# Cari index tower berdasarkan scene name atau tipe
...
func handle_tile_click(mouse_pos: Vector2):
	...
	# Cek apakah ada tower di posisi ini
	...
	# Jika tidak ada tower, lakukan logika placement seperti biasa
...
func place_tower_at_grid(grid_pos: Vector2i, tower_index: int = 0):
	...
	# Simpan reference tower instance di dictionary
